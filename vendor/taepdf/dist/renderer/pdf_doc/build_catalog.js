// renderer/pdf_doc/build_catalog.ts
...
function putCatalog(ctx, structTreeRootId, pdfaExtras) {
  ...
  ctx.out("<<");
  ctx.out(`/Producer ${ctx.strLit(`taepdf ${pkg.version}`)}`);
  ctx.out(`/CreationDate ${ctx.strLit(ctx.creationDate)}`);
  for (const [k, v] of ctx.metadata)
    ctx.out(`/${toPdfName(k)} ${ctx.strLit(v)}`);
  ctx.out(">>");
  ctx.out("endobj");
  let namesObjId = null;
  ...
  if (ctx.formFieldObjIds.length) {
    ctx.out(
      `/AcroForm << /Fields [${ctx.formFieldObjIds.map((id) => `${id} 0 R`).join(" ")}] /DR ${ctx.resourceDictObjId} 0 R /NeedAppearances false >>`,
    );
  }
  ctx.out(">>");
  ctx.out("endobj");
...
function buildXrefStream(ctx, catalogId, encryptId, infoId) {
  ...
  for (const [oid, stmId, idx] of ctx.objStmMembers)
    stmMap.set(oid, [stmId, idx]);
...
export { buildXrefStream, putCatalog, putEncryptDict };
